...
%
% Code for generating Table 7 (Gaussian matrix) in the paper (upper bound on
% alpha_k)
%-------------------------------------

addpath('./00_Rawdata/01_Low_dimension/01_Gaussian/01_rho_0.5');
addpath('./00_Rawdata/01_Low_dimension/01_Gaussian/02_rho_0.6');
addpath('./00_Rawdata/01_Low_dimension/01_Gaussian/03_rho_0.7');
addpath('./00_Rawdata/01_Low_dimension/01_Gaussian/04_rho_0.8');
addpath('./01_JNcode');
addpath('./02_Pick-l');
...
filename=strcat('T7_NSP_LD_comp_Gaussian-',datestmp);
...
for irho=1:4
    switch irho
        case 1
            msize='_20x40_Gauss.mat';
        case 2
            msize='_24x40_Gauss.mat';
        case 3
            msize='_28x40_Gauss.mat';
        case 4
            msize='_32x40_Gauss.mat';
    end
    for icase=1:10
        scase=num2str(icase);
        if icase ~= 10
        ...
        for kv=1:5
            ...
            % Display results
            resbuf=[irho,icase,kv,alphaPICK1,alphaPICK2,alphaPICK3,alphaTSA_LB_P1,alphaTSA_UB_P1,alphaTSA_LB_P2,alphaTSA_UB_P2,alphaTSA_LB_P3,alphaTSA_UB_P3,alphaLP,alphaSDP,T1pick1,T1pick2,T1pick3,T1tsaP1,T1tsaP2,T1tsaP3,T1lp,T1sdp];
            disp('**************************************************************************************************');
            disp(num2str(resbuf));
            disp('**************************************************************************************************');
            count=count+1;
